...
class RINEXReport:
    ...
    def generate(self, output_dir="professional_rinex_report"):
        ...
        for row in quality["sat_scores"].iter_rows(named=True):
            icon = "✅" if row["rating"] == "Excellent" else "⚠️" if row["rating"] == "Fair" else "❌"
        ...
        for const in constellations:
            ...
            self.plotter.plot_constellation_histograms(const, 
                                                    sorted(self.analyzer.df.filter(pl.col("constellation") == const)["frequency"].unique().to_list()), 
                                                    hist_path)
            ...
            bands = sorted(self.analyzer.df.filter(pl.col("constellation") == const)["frequency"].unique().to_list())
            for band in bands:
                print(f"  - Detailed plots for {cname} {band}...")
                sats = sorted(self.analyzer.df.filter((pl.col("constellation") == const) & (pl.col("frequency") == band))["satellite"].unique().to_list())
