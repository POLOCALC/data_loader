...
from pils.tools import get_logpath, read_log_time
...
class Camera:
    ...
    def load_data(self):
        ...
        duration_sec = self.capture.get(cv2.CAP_PROP_FRAME_COUNT) / self.capture.get(
            cv2.CAP_PROP_FPS
        )
...
def detect_peaks(data, column, negate=False, width=1.0, prominence=1.0):
    sign = -1 if negate is True else 1
    peaks, _ = sp.signal.find_peaks(
        sign * data[column], width=width, prominence=prominence
    )
...
def get_scanning_range(DH, margin=15, plot=False):
    ...
    alt_peaks = detect_peaks(
        drone_data, "RTKdata:Hmsl_P", negate=True, width=5, prominence=1e-7
    )
    lat_peaks = detect_peaks(
        drone_data, "RTKdata:Lat_P", negate=True, width=10, prominence=1e-4
    )
    lon_peaks = detect_peaks(
        drone_data, "RTKdata:Lon_P", negate=True, width=10, prominence=1e-4
    )

    # Inclino peaks
    inclino_data = get_flying_range(DH.payload.inclino.data, DH.litchi)
    inclino_peaks = detect_peaks(
        inclino_data, "pitch", negate=True, width=50, prominence=3
    )

    # Barometer peaks
    baro_data = get_flying_range(DH.payload.baro.data, DH.litchi)
    baro_peaks = detect_peaks(
        baro_data, "pressure", negate=False, width=1, prominence=2
    )
    ...
    for i, t in enumerate(seconds):
        ...
        twin = 5
        if (
            np.any(np.abs(seconds - t) <= twin)
            and np.sum(np.abs(seconds - t) <= twin) > 1
        ):
            keep_mask[i] = True
    ...
    if not consensus_peaks.empty:
        ...
        scan_start, scan_end = scanning_range[0] - pd.Timedelta(
            seconds=margin
        ), scanning_range[1] + pd.Timedelta(seconds=margin)
    else:
        scanning_range = None
