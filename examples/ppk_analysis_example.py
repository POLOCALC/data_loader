...
def example_position_quality_analysis():
    ...
    print(f"  Fixed solutions: {len(fixed_solutions)} ({len(fixed_solutions)/len(pos_df)*100:.1f}%)")
    print(f"  Float solutions: {len(float_solutions)} ({len(float_solutions)/len(pos_df)*100:.1f}%)")
...
def example_complete_workflow():
    ...
    print(f"  Time overlap: {max(payload_gps['timestamp'].min(), ppk_pos['timestamp'].min())} to {min(payload_gps['timestamp'].max(), ppk_pos['timestamp'].max())}")
...
def example_batch_processing():
    ...
    for result in results:
        print(f"{result['flight']:30s} {result['epochs']:6d} epochs  {result['fixed_ratio']*100:5.1f}% fixed")
