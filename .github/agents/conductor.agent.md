...
tools: ['execute/testFailure', 'execute/getTerminalOutput', 'execute/runTask', 'execute/createAndRunTask', 'execute/runInTerminal', 'read/problems', 'read/readFile', 'read/terminalSelection', 'read/terminalLastCommand', 'edit', 'search', 'agent', 'todo']
...
2. **Check for Data Structure Work**: If the request involves creating data structure (save data or create data container):
   - Use #runSubagent to invoke the data-subagent first
   - Provide the data structure
   - The data-creator will create a design brief in md format
   - This date structure brief will be referenced in the plan
...
**data-subagent**:
- Invoke BEFORE planning when data structure work is required
- Provide the data structure requirements and any preferences mentioned by user
- Instruct to research existing data structures in the project
- Tell them to create a design brief in MD format
